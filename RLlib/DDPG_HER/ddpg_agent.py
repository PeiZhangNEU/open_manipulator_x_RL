...
class ddpg_agent:
    ...
    def _select_actions(self, pi):
        ...
        action += np.random.binomial(1, self.args.random_eps, 1)[0] * (random_actions - action)   # 有0.9 的概率生成0，此时： action = action + 0 * (random_actions - action) = action           也就是说有0.9的概率是policy选动作
                                                                                                  # 有0.1 的概率生成1，此时： action = action + 1 * (random_actions - action) = random_actions   0.1的概率是随机动作。这个0.1就是设置中的random_ep
        return action
